...
if __name__ == "__main__":
    ...
    bnet.load_state_dict(torch.load("Networks/data_net_low_noise.pth"))
    bnet.eval()
    ...
    for b in range(nb):
        ...
        np.save("Recons/type_d_dc_recon_low_noise.npy", x_np)
    ...
    for b in range(nb):
        ...
        xc = torch.clamp( mask*(x-1.5)+ 1.5, 1.4,1.6)
        x_np[b::nb,:,:,:] = xc.cpu().detach().numpy()
        np.save("Recons/other_dc_recon_low_noise.npy", x_np)"""
    ...
    for b in range(nb):
        ...
        xc = torch.clamp( mask*(x-1.5)+ 1.5, 1.4,1.6)
        x_np[b::nb,:,:,:] = xc.cpu().detach().numpy()
        np.save("Recons/type_d_dc_recon_low_medium.npy", x_np)
    ...
    for b in range(nb):
        ...
        xc = torch.clamp( mask*(x-1.5)+ 1.5, 1.4,1.6)
        x_np[b::nb,:,:,:] = xc.cpu().detach().numpy()
        np.save("Recons/other_dc_recon_low_medium.npy", x_np)
    ...
    for b in range(nb):
        ...
        xc = torch.clamp( mask*(x-1.5)+ 1.5, 1.4,1.6)
        x_np[b::nb,:,:,:] = xc.cpu().detach().numpy()
        np.save("Recons/type_d_dc_recon_low_high.npy", x_np)"""
    ...
    for b in range(nb):
        ...
        xc = torch.clamp( mask*(x-1.5)+ 1.5, 1.4,1.6)
        x_np[b::nb,:,:,:] = xc.cpu().detach().numpy()
        np.save("Recons/other_dc_recon_low_high.npy", x_np)