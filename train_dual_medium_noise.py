...
if __name__ == "__main__":
    ...
    del c_np2
    x_np2 = np.load("Recons/other_dc_recon_medium_noise.npy")
    ...
    for e in range(nepochs):
        ...
        if e > 0 and e%n_checkpoint == 0:
            np.save("Losses/dual_net_medium_noise_losses.npy", np.array(losses))
            plt.clf()
            plt.semilogy(np.linspace(0, e+1, len(losses)), losses)
            plt.savefig("losses.png")
            torch.save(anet.state_dict(), "Networks/dual_net_medium_noise.pth")
    np.save("Losses/dual_net_medium_noise_losses.npy", np.array(losses))
    plt.clf()
    plt.semilogy(np.linspace(0, e+1, len(losses)), losses)
    plt.savefig("losses.png")
