...
if __name__ == "__main__":
    ...
    #tnet = TumorNet(4, 4, 4, 4).to(dev)
    train_ratio = 20/35


    c_np = np.load("Recons/type_d_ac_recon_high_noise.npy")
    ...
    c_np2 = np.load("Recons/other_ac_recon_high_noise.npy")
    ...
    for e in range(nepochs):
        ...
        for b in batch_inds:
            ...
            true_loss = -torch.mean( T*torch.log(Y + eps ))
            false_loss = -torch.mean( (1-T)*torch.log(1-Y + eps))
            #rrmse = torch.mean( (T-(Y > 0.5))**2)**(1/2)/torch.mean(T**2)**(1/2)
            #dice = torch.sum(2*T*(Y > 0.5))/torch.sum(T + (Y > 0.5))
            ...
            loss.backward()
            opt.step()
            #print(e, true_loss.item(), false_loss.item(), loss.item(), rrmse.item(), (Y.mean()/T.mean()).item(), dice.item())
            #print((torch.mean(Y*T)/torch.mean(T)).item())

        if e > 0 and e%n_checkpoint == 0:
            #np.save("Losses/art_net_high_noise_losses.npy", np.array(losses))
            plt.clf()
            plt.semilogy(np.linspace(0, e+1, len(losses)), losses)
            plt.savefig("losses.png")
            torch.save(tnet.state_dict(), "Tumor_Segmenters/ac_net_high_noise.pth")
    torch.save(tnet.state_dict(), "Tumor_Segmenters/ac_net_high_noise.pth")
