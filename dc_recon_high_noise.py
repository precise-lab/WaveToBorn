...
if __name__ == "__main__":
    ...
    bnet.load_state_dict(torch.load("Networks/data_net_high_noise.pth"))
    bnet.eval()
    ...
    for b in range(nb):
        ...
        np.save("Recons/type_d_dc_recon_high_noise.npy", x_np)
    ...
    for b in range(nb):
        ...
        xc = torch.clamp( mask*(x-1.5)+ 1.5, 1.4,1.6)
        x_np[b::nb,:,:,:] = xc.cpu().detach().numpy()
        np.save("Recons/other_dc_recon_high_noise.npy", x_np)
    ...
    for b in range(nb):
        ...
        xc = torch.clamp( mask*(x-1.5)+ 1.5, 1.4,1.6)
        x_np[b::nb,:,:,:] = xc.cpu().detach().numpy()
        np.save("Recons/type_d_dc_recon_high_low.npy", x_np)
    ...
    for b in range(nb):
        ...
        xc = torch.clamp( mask*(x-1.5)+ 1.5, 1.4,1.6)
        x_np[b::nb,:,:,:] = xc.cpu().detach().numpy()
        np.save("Recons/other_dc_recon_high_low.npy", x_np)
    ...
    for b in range(nb):
        ...
        xc = torch.clamp( mask*(x-1.5)+ 1.5, 1.4,1.6)
        x_np[b::nb,:,:,:] = xc.cpu().detach().numpy()
        np.save("Recons/type_d_dc_recon_high_medium.npy", x_np)
    ...
    for b in range(nb):
        ...
        xc = torch.clamp( mask*(x-1.5)+ 1.5, 1.4,1.6)
        x_np[b::nb,:,:,:] = xc.cpu().detach().numpy()
        np.save("Recons/other_dc_recon_high_medium.npy", x_np)