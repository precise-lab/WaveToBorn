...
if __name__ == "__main__":
    ...
    anet.load_state_dict(torch.load("Networks/art_net_high_noise.pth"))
    anet.eval()
    ...
    np.save("Recons/type_d_ac_recon_high_noise.npy", y_np)
    ...
    for b in range(nb):
        ...
        Y = torch.clamp(Y, 1.4, 1.6)
        y_np[b::nb,:,3:-3,3:-3] = Y.cpu().detach().numpy()
    np.save("Recons/other_ac_recon_high_noise.npy", y_np)
    ...
    for b in range(nb):
        ...
        Y = torch.clamp(Y, 1.4, 1.6)
        y_np[b::nb,:,3:-3,3:-3] = Y.cpu().detach().numpy()
    np.save("Recons/type_d_ac_recon_high_low.npy", y_np)
    ...
    for b in range(nb):
        ...
        Y = torch.clamp(Y, 1.4, 1.6)
        y_np[b::nb,:,3:-3,3:-3] = Y.cpu().detach().numpy()
    np.save("Recons/other_ac_recon_high_low.npy", y_np)
    ...
    for b in range(nb):
        ...
        Y = torch.clamp(Y, 1.4, 1.6)
        y_np[b::nb,:,3:-3,3:-3] = Y.cpu().detach().numpy()
    np.save("Recons/type_d_ac_recon_high_medium.npy", y_np)
    ...
    for b in range(nb):
        ...
        Y = torch.clamp(Y, 1.4, 1.6)
        y_np[b::nb,:,3:-3,3:-3] = Y.cpu().detach().numpy()
    np.save("Recons/other_ac_recon_high_medium.npy", y_np)
