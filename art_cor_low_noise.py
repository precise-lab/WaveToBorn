...
if __name__ == "__main__":
    ...
    anet.load_state_dict(torch.load("Networks/art_net_low_noise.pth"))
    anet.eval()
    ...
    np.save("Recons/type_d_ac_recon_low_noise.npy", y_np)
    ...
    for b in range(nb):
        ...
        Y = torch.clamp(Y, 1.4, 1.6)
        y_np[b::nb,:,3:-3,3:-3] = Y.cpu().detach().numpy()
    np.save("Recons/other_ac_recon_low_noise.npy", y_np)
    ...
    for b in range(nb):
        ...
        Y = torch.clamp(Y, 1.4, 1.6)
        y_np[b::nb,:,3:-3,3:-3] = Y.cpu().detach().numpy()
    np.save("Recons/type_d_ac_recon_low_medium.npy", y_np)
    ...
    for b in range(nb):
        ...
        Y = torch.clamp(Y, 1.4, 1.6)
        y_np[b::nb,:,3:-3,3:-3] = Y.cpu().detach().numpy()
    np.save("Recons/other_ac_recon_low_medium.npy", y_np)
    ...
    for b in range(nb):
        ...
        Y = torch.clamp(Y, 1.4, 1.6)
        y_np[b::nb,:,3:-3,3:-3] = Y.cpu().detach().numpy()
    np.save("Recons/type_d_ac_recon_low_high.npy", y_np)
    ...
    for b in range(nb):
        ...
        Y = torch.clamp(Y, 1.4, 1.6)
        y_np[b::nb,:,3:-3,3:-3] = Y.cpu().detach().numpy()
    np.save("Recons/other_ac_recon_low_high.npy", y_np)
