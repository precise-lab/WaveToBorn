...
if __name__ == "__main__":
    with torch.no_grad():
        ...
        inet.load_state_dict(torch.load("Networks/inet_high_noise.pth"))
        inet.eval()
        ...
        for b in range(nb):
            ...
            np.save("Recons/type_d_inet_recon_high_noise.npy", x_np)
        nb *= 8
        ...
        for b in range(nb):
            p = torch.from_numpy(p_np[b::nb,:,:,-496:]).to(dev)
            x_np[b::nb,:, 3:-3, 3:-3] = inet(p).cpu().detach().numpy()
            np.save("Recons/other_inet_recon_high_noise.npy", x_np)
        ...
        for b in range(nb):
            p = torch.from_numpy(p_np[b::nb,:,:,-496:]).to(dev)
            x_np[b::nb, :, 3:-3, 3:-3] = inet(p).cpu().detach().numpy()
            np.save("Recons/type_d_inet_recon_high_low.npy", x_np)
        nb *= 8
        p_np = np.load("Measurements/other_wave_offset.npy")
        p_np += std*np.random.standard_normal(p_np.shape)
        x_np = 0*np.load("Initial_Guesses/other_initial_guess.npy")+ 1.5
        ...
        for b in range(nb):
            p = torch.from_numpy(p_np[b::nb,:,:,-496:]).to(dev)
            x_np[b::nb,:, 3:-3, 3:-3] = inet(p).cpu().detach().numpy()
            np.save("Recons/other_inet_recon_high_low.npy", x_np)
        ...
        for b in range(nb):
            p = torch.from_numpy(p_np[b::nb,:,:,-496:]).to(dev)
            x_np[b::nb, :, 3:-3, 3:-3] = inet(p).cpu().detach().numpy()
            np.save("Recons/type_d_inet_recon_high_medium.npy", x_np)
        nb *= 8
        p_np = np.load("Measurements/other_wave_offset.npy")
        p_np += std*np.random.standard_normal(p_np.shape)
        x_np = 0*np.load("Initial_Guesses/other_initial_guess.npy")+ 1.5
        ...
        for b in range(nb):
            p = torch.from_numpy(p_np[b::nb,:,:,-496:]).to(dev)
            x_np[b::nb,:, 3:-3, 3:-3] = inet(p).cpu().detach().numpy()
            np.save("Recons/other_inet_recon_high_medium.npy", x_np)
